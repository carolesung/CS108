...
def doHTMLTail():
    ...
    <a href="./miniFacebook4.py"> Return to main page.</a>
    </body>
    </html>
...
def showProfilePage(data):
    ...
    """% (firstName, lastname, profileid, email, activities, profileid))
    
    d = getFriends(profileid)
    showFriends(d)
    print("""
    <form>
        <input type="text" name="friendID">
        <input type="hidden" name="profileID" value=%s>
        <input type="submit" name="addNewFriend" value="Add friend">
    </form>
...
def getFriends(profileID):
    '''Obtains the list of friends of profileID given.'''
    ...
    # build SQL
    sql = """
    SELECT profiles.profileid,lastname,firstname FROM profiles INNER JOIN friends
    ON profiles.profileid = friends.friendid
    WHERE friends.profileid = %s"""
    ...
    return data
##################
def showFriends(data):
    '''Takes list of tuples containing friends' records and prints out a table
    or list of friends, with each friend's name and a link to load that
    person's profile.'''
        ## create an HTML table for output:
    print("""
    <h2>Friend List</h2>
...
def addFriend(profileID,friendID):
    '''take two parameters: the profileID of the user for whom we are
    adding a friend, and friendID, which is the profile id of the friend
    to be added.'''
    ...
    # build SQL
    sql = """
    INSERT INTO friends (profileID,friendID) VALUES (%s,%s)"""
...
#################
def showUpdateProfileForm(profiledata):
    '''Display an HTML form to update the profile for a user.'''
    (profileid, lastname, firstName, email, activities) = profiledata[0]
    print("""
    <form>
    <table style="width:80%">
        <tr>
	       <td><b>Last Name:</b></td>
	       <td><input type="text" name="lastName" value= {0!s}></td>
        </tr>
        <tr>
	       <td><b>First Name:</b></td>
	       <td><input type="text" name="firstName" value= {1!s}></td>
        </tr>
        <tr>
	       <td><b>Email:</b></td>
	       <td><input type="text" name="email" value= {2!s}></td>
        </tr>
        <tr>
	       <td><b>Activities:</b></td>
	       <td><input type="text" name="activities" size="25" value= {3!s}></td>
        </tr>
        <tr>
            <td><input type="submit" name="Update" value="Submit">
            <td><input type="hidden" name="profileID" value= {4!s}>
        </tr>
    </table>
    </form>""".format(lastname, firstName, email, activities, profileid))

#################
def updateProfile(profileID,lastname,firstname,email,activities):
    '''Encapsulates the SQL to do an UPDATE query to modify the profile.
    The update replaces the existing email and activities for the profile
    record corresponding to profileID.'''
    ...
    firstname=%s,
    email=%s,
    activities=%s WHERE profileid = %s;"""
...
##################
def getNewsFeed(profileID):
    '''Return all news feed items that should show for the user
with the profileID'''

##################
def delete():
    '''Deletes user profiles with specified profileid.'''
    ...
    # build SQL
    sql = """
    DELETE FROM profiles WHERE profileid in (9,10,11,12,13,14)"""
...
if __name__ == "__main__":
    ...
    if "profileid" in form:
        ...
        profileid = form["profileID"].value
        profiledata = getOneProfile(profileid)
        showUpdateProfileForm(profiledata)
        ...
        email = form["email"].value
        activities = form["activities"].value
        updateProfile(profileid,last,first,email,activities)
        profiledata = getOneProfile(profileid)
        showProfilePage(profiledata)
